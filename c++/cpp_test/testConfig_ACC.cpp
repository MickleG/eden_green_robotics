...
{   
    ...
    public:
        ...
        void setSpeed(float speed) // use value from -160 to 160 to represent mm/s speed -- deadband -1 to +1 -- see motorConfig.pdf for speed charts
        {
        ...
        void controlLoop()
        {
            ...
            switch (driveState)
            {
                case 1: // normal driving operation
                ...
                case -1: // if limitOutside is pressed
                    ...
                    if (!digitalRead(limitOutside)) // if limitOutside remains pressed
                    {
                        ...
                        if (!digitalRead(limitInside)) {driveState = 0;}
                ...
                case 2: // if limitinside is pressed
                    ...
                    if (!digitalRead(limitInside)) // if limitInside remains pressed
                    {
                        ...
                        if (!digitalRead(limitOutside)) {driveState = 0;}
...
class EndEffectorConfig 
{
    ...
    public:
        ...
        void calibrateZero(float speed) // NEEDS UPDATED FOR ACCELERATION
        {
            ...
            while (digitalRead(rightMotor.limitOutside) && digitalRead(leftMotor.limitInside)) // check right switch
            {
            ...
            while (digitalRead(leftMotor.limitOutside) && digitalRead(rightMotor.limitInside))
            {
...
int main()
{
    ...
    /*
    while (digitalRead(mechanism.leftMotor.limitOutside) && digitalRead(mechanism.rightMotor.limitInside))
    {
        printf("reeee\n");
    }
    */
    ...
    mechanism.goToPosition(-40, 200, 140);
    mechanism.goToPosition(0, 100, 140);
    mechanism.goToPosition(40, 200, 140);
    mechanism.goToPosition(0, 100, 140);
    ...
    float distX = 30;
    float distZ = 100;
    float overallSpeed = 60;

    
    while(getNanos() - time1 < 1000000000)
    {

        mechanism.directionalDrive(-distX,distZ,overallSpeed);
        //printf("Drive State Left: %d\n", mechanism.rightMotor.driveState);
    }
